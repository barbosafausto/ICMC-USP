...
$$
\displaystyle \int_a^b \Big( \int_c^d f(x, y)dx \Big) dy = \sum_{j=1}^m\Big(\sum_{i=1}^nf(x, y)\Delta x_i\Big) \Delta y_j
$$
...
$$
\displaystyle \int\int_D f(x)dx = \int_a^b \Big( \int_{g_1(x)}^{g_2(x)} f(x)dy \Big) dx
$$
